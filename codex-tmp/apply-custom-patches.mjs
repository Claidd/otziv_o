...
if (!['--check', '--apply'].includes(mode) || args.length === 0) {
  throw new Error('Usage: node apply-custom-patches.mjs --check|--apply PATCH...');
}
...
function safePath(relative) {
  ...
  if (absolute !== root && !absolute.startsWith(prefix)) {
    throw new Error(`Patch path escapes workspace: ${relative}`);
  }
  return absolute;
}
...
function startsAt(lines, index, expected) {
...
function findMatch(lines, expected, cursor, label) {
  ...
  for (let index = cursor; index <= lines.length - expected.length; index += 1) {
    if (startsAt(lines, index, expected)) return index;
  }
  throw new Error(`Hunk context not found after line ${cursor + 1}: ${label}\n${expected.slice(0, 8).join('\n')}`);
}
...
function parseHunks(sectionLines, label) {
  ...
  for (const line of sectionLines) {
    ...
    if (line.startsWith(' ')) {
      ...
      // Not used by these patches, accepted for completeness.
    } else {
...
function applyUpdate(relative, sectionLines, patchName) {
  ...
  for (let i = 0; i < hunks.length; i += 1) {
    ...
    target.lines.splice(index, hunk.oldLines.length, ...hunk.newLines);
...
function applyAdd(relative, sectionLines, patchName) {
  const absolute = safePath(relative);
  if (fs.existsSync(absolute) || state.has(relative)) {
    throw new Error(`Add target already exists: ${patchName}:${relative}`);
  }
  const content = [];
  for (const line of sectionLines) {
...
function applyPatchFile(patchFile) {
  ...
  const raw = fs.readFileSync(path.resolve(root, patchFile), 'utf8').replaceAll('\r\n', '\n');
  const lines = raw.split('\n');
  if (lines.at(-1) === '') lines.pop();
  if (lines.shift() !== '*** Begin Patch' || lines.pop() !== '*** End Patch') {
    throw new Error(`Invalid patch sentinels: ${patchName}`);
  }
  let index = 0;
  while (index < lines.length) {
    ...
    index += 1;
    const section = [];
    while (index < lines.length && !lines[index].startsWith('*** Update File: ') && !lines[index].startsWith('*** Add File: ')) {
...
process.stdout.write(`${mode === '--apply' ? 'Applied' : 'Checked'} ${args.length} patches; ${touched.size} files matched exactly.\n`);
