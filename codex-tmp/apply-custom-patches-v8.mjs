...
if (argumentIndex >= 0) {
  ...
  const staleTestSection = /\*\*\* Update File: backend\/src\/test\/java\/com\/hunt\/otziv\/payments\/service\/ManualPaymentTaskLedgerServiceTest\.java\n[\s\S]*?(?=\*\*\* End Patch)/;
  if (!staleTestSection.test(patch)) throw new Error('Expected stale test section was not found');
  patch = patch.replace(staleTestSection, '');
