...
const ARCHIVE_HELP = {
  ...
  batchRows: 'Сумма связанных строк: детали, отзывы, задачи плохих отзывов, заявки, начисления и оплаты.',
...
const ARCHIVE_MANUAL_SECTIONS: ArchiveManualSection[] = [
  ...
  },
  {
    ...
    points: [
      ...
      'Размер пачки ограничивает только число заказов. Отзывы, детали, начисления, оплаты и другие связанные строки подтягиваются к выбранным заказам автоматически.'
    ]
  },
  {
    title: '4. Dry-run и Live',
    points: [
      'Dry-run ничего не переносит и не удаляет. Он только считает: сколько заказов, деталей, отзывов, начислений и оплат попало бы в пачку.',
