...
- The VNC URL is returned only in the no-store open response. It is not persisted or logged. Provider messages and external keys are also excluded from lifecycle logs.
...
- `POST /api/bots/{botId}/browser/open` accepts `{ "heartbeatSupported": true }` and returns `sessionId`, `vncUrl`, `heartbeatIntervalSeconds`, `expiresAt`, plus backward-compatible display fields.
