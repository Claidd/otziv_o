...
import com.hunt.otziv.archive.dto.ArchiveOrderCandidateItem;
...
public class OrderArchiveDryRunRepository {
    ...
    public long countMissingClosedAnalyticsMonths(LocalDate cutoffDate, int batchLimit, LocalDate currentMonthStart) {
        MapSqlParameterSource params = candidateParams(cutoffDate, batchLimit)
    ...
    public long countMissingClosedAnalyticsMonthsForPreparedCandidates(LocalDate currentMonthStart) {
        jdbc.update("""
    ...
    public Long insertStartedArchiveBatch(
            LocalDateTime startedAt,
            String archiveReason,
            int retentionDays,
            ArchiveCandidateCounts selected,
    ...
    public List<ArchiveBatchSummary> findLatestBatches(int limit) {
        return jdbc.query("""
                SELECT
                    archive_batch_id,
                    started_at,
    ...
    public ArchiveBatchSummary findBatch(Long batchId) {
    ...
    public List<ArchiveOrderCandidateItem> findArchivedOrdersByBatch(Long batchId, int limit) {
        return jdbc.query("""
                SELECT
                    ao.order_id,
                    ao.order_company,
    ...
    public boolean tryAcquireArchiveLock(String lockName, int timeoutSeconds) {
    ...
    public void releaseArchiveLock(String lockName) {
        jdbc.queryForObject("SELECT RELEASE_LOCK(:lockName)", Map.of("lockName", lockName), Long.class);
    }

    public ArchiveLockStatus lockStatus(String lockName) {
