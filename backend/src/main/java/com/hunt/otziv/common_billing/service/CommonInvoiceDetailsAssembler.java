...
import com.hunt.otziv.p_products.api.NextOrderRequests.CreatedOrder;
...
public class CommonInvoiceDetailsAssembler {
    ...
    List<CommonInvoiceNextCycleResponse> toNextCycleOrders(List<CommonInvoiceOrder> items) {
        ...
        List<CreatedOrder> requests = nextOrderRequests.createdOrdersForSources(sourceOrderIds);
        ...
        return requests.stream().map(request -> toNextCycleResponse(request,
                request.orderId() == null ? null : memberships.get(request.orderId()))).toList();
    }

    private Map<Long, CurrentOrderInvoiceView> currentInvoiceMemberships(List<CreatedOrder> requests) {
        List<Long> createdOrderIds = requests.stream().map(CreatedOrder::orderId)
    ...
    private CommonInvoiceNextCycleResponse toNextCycleResponse(CreatedOrder request, CurrentOrderInvoiceView linkedInvoice) {
        return new CommonInvoiceNextCycleResponse(request.sourceOrderId(), request.orderId(),
                linkedInvoice == null ? null : linkedInvoice.getInvoiceId(),
                linkedInvoice == null ? null : linkedInvoice.getInvoiceStatus().name(),
                request.companyTitle(), request.filialTitle(), request.statusTitle());
    }
