...
public class WorkerCellularAccessService {
    ...
    public WorkerCellularAccessService(WorkerCellularAccessProperties properties) {
        this.properties = properties;
        this.cidrMatcher = new IpCidrMatcher(properties.getAllowedCidrs());
        if (properties.getMode() != WorkerCellularAccessProperties.Mode.OFF && cidrMatcher.isEmpty()) {
    ...
    public void enforceProtectedAccess(String scope) {
        ...
        boolean cellularNetwork = cidrMatcher.matches(clientIp);
        boolean allowed = mobileDevice && cellularNetwork;
        String reason = !mobileDevice ? "DESKTOP_OR_UNKNOWN_DEVICE" : !cellularNetwork ? "NON_CELLULAR_OR_UNKNOWN_IP" : "ALLOWED";

        log.info(
                "Worker cellular access: user={}, scope={}, mode={}, result={}, reason={}, mobileDevice={}, cidrMatch={}, ipPrefix={}",
