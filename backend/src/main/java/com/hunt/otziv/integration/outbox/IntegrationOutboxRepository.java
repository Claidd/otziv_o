...
class IntegrationOutboxRepository {
    ...
    IntegrationOutboxRepository(NamedParameterJdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    boolean markSucceeded(long outboxId, String processingToken) {
    ...
    boolean markDead(long outboxId, String processingToken, String lastError) {
        MapSqlParameterSource parameters = fence(outboxId, processingToken)
                .addValue("lastError", lastError, Types.VARCHAR);
        return jdbc.update(DEAD_SQL, parameters) == 1;
    }

    int markExpiredExhaustedDead(
            int rowLimit,
            Collection<String> allowedEventTypes,
            String lastError
    ) {
    ...
    StatusSnapshot statusSnapshot(int countCap) {
    ...
    record CountSample(long value, boolean capped) {
    }
