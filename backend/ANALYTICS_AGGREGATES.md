...
- `cabinet/analyse`: зарплаты из канонического `analytics_salary_source`, чеки из
  `analytics_payment_source`.
- `cabinet/team`: пользователи, роли, менеджеры/работники плюс месячные суммы зарплат из того же канонического источника.
- Рейтинг: каноническая зарплата, `analytics_payment_source`, новые компании, новые заказы,
  опубликованные/ожидающие отзывы, лиды.
- Личный кабинет работника: дневные и месячные показатели из окончательного распределения начислений.

`analytics_salary_source` исключает неактивные `zp` и использует активный
`contractor_reward_ledger`, если начисление уже синхронизировано. Поэтому отмена работы и
перераспределение между специалистами одинаково отражаются в расчётном профиле, рейтинге и
личном кабинете. Активные legacy-строки без ledger остаются безопасным fallback.

Обе финансовые view archive-aware: `analytics_payment_source` объединяет активные строки
`payment_check` и `archive_payment_check`, а `analytics_salary_source` добавляет legacy-начисления
из `archive_zp`, которых нет ни в live-таблице, ни в ledger. При восстановлении заказа live-строка
имеет приоритет по первичному ключу, поэтому двойного учета нет. Это обязательный инвариант:
пересборка закрытого месяца после архивирования не должна занулять историческую аналитику.
...
3. Для персональных и командных карточек текущего месяца выполнить один пакетный live-досчет из
   `analytics_salary_source`. Это один запрос на экран, а не N запросов по сотрудникам.
...
- зарплаты и количество оплаченных отзывов из `analytics_salary_source`;
- суммы и количество чеков из `analytics_payment_source`;
