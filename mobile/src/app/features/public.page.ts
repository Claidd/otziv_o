...
const PAGES: Record<PublicPageKey, PublicPageContent> = {
  services: {
    eyebrow: 'Что продаем',
    title: 'Услуги',
    lead: 'Информационно-маркетинговые услуги для компаний: репутационное сопровождение, тексты, карточки организаций и работа с обратной связью.',
  ...
  payment: {
    ...
    accent: 'Доступны банковская карта, СБП или ручной способ оплаты, если он включен для заказа.',
    sections: [
      ...
      { icon: 'looks_two', title: 'Платежная ссылка', body: 'В ссылке отображаются компания, заказ, услуга и итоговая сумма.' },
  ...
  offer: {
    ...
    lead: `Оферта определяет условия оказания услуг ${BUSINESS.shortName}. Оплата заказа означает согласие клиента с условиями оферты.`,
    accent: `${BUSINESS.officialName}, ИНН ${BUSINESS.inn}, ОГРНИП ${BUSINESS.ogrnip}.`,
    sections: [
      { icon: 'badge', title: 'Стороны договора', body: `Исполнитель: ${BUSINESS.officialName}, адрес: ${BUSINESS.address}. Заказчик: лицо, заказавшее и оплатившее услугу.` },
      { icon: 'rule', title: 'Предмет оферты', body: 'Исполнитель оказывает информационно-маркетинговые услуги по согласованному заданию клиента.' },
      { icon: 'receipt_long', title: 'Расчеты и НДС', body: `СНО: ${BUSINESS.taxSystem}. Операции оформляются без НДС.` }
    ]
  },
  privacy: {
    eyebrow: '152-ФЗ',
  ...
  receiptConsent: {
    ...
    sections: [
      ...
      { icon: 'fact_check', title: 'Связь с заказом', body: 'Платежная ссылка связывает оплату и чек с услугой, заказом или компанией клиента.' }
    ]
  },
  pay: {
    eyebrow: 'Форма',
