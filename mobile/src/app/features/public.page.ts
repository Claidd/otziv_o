...
const BUSINESS = {
  ...
  address: 'Россия, Иркутская область, СНТ Надежда, ул. 5-я Летняя, д. 222',
...
const PAGES: Record<PublicPageKey, PublicPageContent> = {
  ...
  privacy: {
    ...
    sections: [
      { icon: 'groups', title: 'Субъекты и данные', body: 'Обрабатываются данные посетителей, клиентов, представителей клиентов, контрагентов и уполномоченных пользователей системы в объеме, необходимом для целей сервиса.' },
  ...
  contacts: {
    ...
    sections: [
      ...
      { icon: 'fact_check', title: 'Вид деятельности', body: 'Информационно-маркетинговые услуги сайта o-ogo.ru соответствуют дополнительным кодам ОКВЭД по ЕГРИП: 73.11, 63.99.1, 63.11.9, 63.12, 62.09, 82.99.' },
