...
export class PublicPayPage {
  ...
  readonly canReportManual = computed(() => Boolean(this.payment()?.payable && this.manualPayment() && this.payment()?.status !== 'MANUAL_REPORTED' && !this.manualSubmitting()));
  readonly manualPaymentButtonLabel = computed(() => this.payment()?.manualPaymentButtonLabel?.trim() || 'Оплатить через Альфа-Банк');
  readonly manualPaymentUrl = computed(() => this.payment()?.manualPaymentUrl?.trim() || 'https://pay.alfabank.ru/sc/EWwpfrArNZotkqOR');
  ...
  submitSbp(): void {
    ...
    this.api.initPublicSbpPayment(this.token(), this.email().trim(), this.offerConsent(), this.privacyConsent(), this.receiptConsent(), this.selectedSbpBankId() || null).subscribe({
      next: (response) => {
        ...
        const target = response.qrPayload || response.paymentUrl;
        if (target) {
  ...
  openManualPaymentUrl(): void {
    window.location.href = this.manualPaymentUrl();
  }
