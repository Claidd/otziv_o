...
export const COMMON_INVOICE_NO_PAYMENT_ACTION_LABEL = 'В банке оплаты нет — продолжить';

export function commonInvoicePaymentInitInstructions(): string[] {
  return [
    'В кабинете банка откройте операции интернет-эквайринга. В поле «Номер заказа» вводите только OrderId.',
...
export function commonInvoiceNoPaymentActionHint(): string {
  return `«${COMMON_INVOICE_NO_PAYMENT_ACTION_LABEL}» подтверждает только отсутствие оплаты `
    + 'по перечисленным OrderId и PaymentId в банке и закрывает техническую ошибку. '
...
export function commonInvoiceNoPaymentConfirmation(evidenceText: string): string {
  return 'РЕЗУЛЬТАТ ПРОВЕРКИ БАНКА: ПО УКАЗАННЫМ ID ОПЛАТЫ НЕТ.\n\n'
    + 'Подтвердите, что вы проверили в банке ВСЕ перечисленные OrderId и PaymentId и ни один платёж по ним '
