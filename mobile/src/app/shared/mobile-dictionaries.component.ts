...
export class MobileDictionariesComponent implements OnInit, OnDestroy {
  ...
  monitorMetrics(): SettingsRow[] {
    ...
    return [
      ...
      { icon: 'healing', title: 'Ждут восстановления', value: String(monitor?.recoveryHoldCandidates ?? 0), detail: 'штатная пауза, не ошибка' },
      { icon: 'support_agent', title: 'Ручной контроль', value: String(monitor?.manualControlCandidates ?? 0), detail: 'требуется вмешательство' },
      { icon: 'replay', title: 'Ждут retry', value: String(monitor?.retryWaitingCandidates ?? 0), detail: 'автоматическая повторная попытка' },
