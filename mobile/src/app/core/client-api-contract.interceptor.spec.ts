...
describe('compiled client SDK through real Angular HTTP transport', () => {
  ...
  it('checks manager pagination and permissions on the actual feature client response', async () => {
    const result = firstValueFrom(TestBed.inject(ManagerBoardApi).getManagerBoard({ section: 'orders', pageNumber: 2, pageSize: 7 }));
    await dispatched();
  ...
  it('preserves a bodyless 204 on the actual status write and sends it once', async () => {
    const result = firstValueFrom(TestBed.inject(ManagerOrdersApi).updateManagerOrderStatus(2, 'В работе'));
    await dispatched();
