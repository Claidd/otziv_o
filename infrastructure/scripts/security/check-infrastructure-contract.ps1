...
Assert-TextMatch $smoke "ACCOUNTING_SHADOW=[\s\S]{0,500}ACCOUNTING_LIVE=[\s\S]{0,500}ROLLOUT_LEGACY=[\s\S]{0,1000}ROLLOUT_COMPLETION=[\s\S]{0,1000}LIVE_ALLOCATIONS=[\s\S]{0,500}CUTOVER_ROWS=" 'Contractor payment smoke must collect enough facts to distinguish pre-cutover and post-cutover rollout states.'
...
Assert-TextMatch $smoke '\$completionLiveState\s*=\s*\([\s\S]{0,1000}\$factValues\["ACCOUNTING_LIVE"\]\s+-eq\s+1[\s\S]{0,1000}\$factValues\["ROLLOUT_COMPLETION"\]\s+-eq\s+1[\s\S]{0,1000}\$factValues\["CUTOVER_ROWS"\]\s+-eq\s+1' 'Contractor payment smoke must accept the immutable post-cutover COMPLETION/LIVE state.'
