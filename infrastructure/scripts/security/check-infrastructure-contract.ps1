...
Assert-TextMatch $backupRestoreDrill '(?m)^#requires -Version 7\.0$' 'The encrypted restore drill must require a PowerShell Core version with AES-GCM support.'
...
Assert-TextMatch $baseEnvExample '(?m)^OTZIV_AUTH_LEGACY_MIGRATION_ENABLED=false$' 'Base env example must keep retired legacy user migration disabled.'
Assert-TextMatch $productionEnvExample '(?m)^OTZIV_AUTH_LEGACY_MIGRATION_ENABLED=false$' 'Production env example must keep retired legacy user migration disabled.'
Assert-TextMatch $localEnvExample '(?m)^OTZIV_AUTH_LEGACY_MIGRATION_ENABLED=false$' 'Prod-local env example must keep retired legacy user migration disabled.'
...
Assert-TextMatch $deploy '(?m)^set_env OTZIV_AUTH_LEGACY_MIGRATION_ENABLED "false"$' 'Production remote deployment must persist the closed legacy migration window.'
Assert-TextMatch $legacyDeploy '(?m)^set_env OTZIV_AUTH_LEGACY_MIGRATION_ENABLED "false"$' 'Legacy deployment must also persist the closed legacy migration window.'
...
Assert-TextMatch $baseEnvExample '(?m)^BACKUP_S3_REQUIRE_SERVER_SIDE_ENCRYPTION=true$' 'Base env example must keep SSE-S3 verification fail closed.'
Assert-TextMatch $productionEnvExample '(?m)^BACKUP_S3_REQUIRE_SERVER_SIDE_ENCRYPTION=true$' 'Production env example must keep SSE-S3 verification fail closed.'
Assert-TextMatch $localEnvExample '(?m)^BACKUP_S3_REQUIRE_SERVER_SIDE_ENCRYPTION=true$' 'Prod-local env example must keep SSE-S3 verification fail closed.'
...
Assert-TextMatch $deploy 'deploy_cleanup[\s\S]{0,1800}self-heal timer remains disabled and stopped[\s\S]{0,250}deploy lock remains' 'A production deployment that fails before payload completion must leave self-heal disabled and retain the durable lock for manual verification.'
...
Assert-TextMatch $whatsAppDockerfile '(?m)^\s*chromium-sandbox \\$' 'The WhatsApp runtime image must install Chromium''s sandbox helper explicitly.'
...
Assert-TextMatch $deploy '(?m)^compose run --rm --no-deps --cap-add CHOWN --user 0 --entrypoint chown app ' 'The one-shot app ownership migration must regain CAP_CHOWN after the service drops all capabilities.'
Assert-TextMatch $deploy '(?m)^compose run --rm --no-deps --cap-add CHOWN --user 0 --entrypoint sh whatsapp_lika ' 'The one-shot WhatsApp ownership migration must regain CAP_CHOWN after the service drops all capabilities.'
Assert-TextMatch $legacyDeploy '(?m)^compose run --rm --no-deps --cap-add CHOWN --user 0 --entrypoint chown app ' 'The legacy one-shot app ownership migration must regain CAP_CHOWN after the service drops all capabilities.'
Assert-TextMatch $legacyDeploy '(?m)^compose run --rm --no-deps --cap-add CHOWN --user 0 --entrypoint sh whatsapp_lika ' 'The legacy one-shot WhatsApp ownership migration must regain CAP_CHOWN after the service drops all capabilities.'
