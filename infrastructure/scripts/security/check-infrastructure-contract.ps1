...
Assert-TextMatch $databaseBackupService 'headObject\(HeadObjectRequest' 'Every uploaded database backup must receive a mandatory HEAD verification.'
...
Assert-TextMatch $smoke 'Sync-LocalKeycloakManagedUsers[\s\S]{0,900}Sync-LocalKeycloakSubjectMappings[\s\S]{0,5000}foreach\s*\(\$databaseUser\s+in\s+@\(\$activeDatabaseUsers' 'All eligible local users and subjects must be synchronized before active login passwords are reset.'
...
Assert-TextMatch $smoke 'foreach \(\$databaseUser in @\(\$activeDatabaseUsers \| Sort-Object Id\)\)' 'Every active eligible local staff/performer identity must receive the isolated local login credential.'
...
foreach ($name in $propertyVariables) {
    if (-not $composeVariables.Contains($name)) {
