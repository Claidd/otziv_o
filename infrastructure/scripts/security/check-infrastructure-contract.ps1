...
foreach ($composeText in @($productionCompose, $localCompose, $developmentCompose)) {
    $ingressBlock = [regex]::Match($composeText, '(?ms)^  nginx:\r?\n(?:(?!^  [A-Za-z0-9_-]+:).)*').Value
    Assert-TextNotMatch $ingressBlock '(?m)^\s{6}(grafana|prometheus|loki|tempo|alloy|dozzle):' 'Ingress startup must remain independent of observability services.'
}
$nginx = Get-RepositoryText 'infrastructure/nginx/prod.conf'
$localNginx = Get-RepositoryText 'infrastructure/nginx/prod-local.conf'
...
$databaseBackupService = Get-RepositoryText 'backend/src/main/java/com/hunt/otziv/s3/backup/service/DatabaseBackupService.java'
$backupScheduler = Get-RepositoryText 'backend/src/main/java/com/hunt/otziv/s3/backup/service/BackupScheduler.java'
$mailConfig = Get-RepositoryText 'backend/src/main/java/com/hunt/otziv/config/email/config/MailConfig.java'
$backupS3Config = Get-RepositoryText 'backend/src/main/java/com/hunt/otziv/s3/backup/config/BackupS3Config.java'
...
Assert-TextMatch $smoke '\$env:OTZIV_CONTRACTOR_PAYMENTS_LIVE_ROUTING_MASTER_ENABLED\s*=\s*''false''' 'Prod-like smoke must disable contractor live-routing master before Compose startup.'
Assert-TextMatch $smoke '\$env:OTZIV_CONTRACTOR_PAYMENTS_REWARD_ATTRIBUTION_MASTER_ENABLED\s*=\s*''false''' 'Prod-like smoke must disable contractor reward master before Compose startup.'
Assert-TextMatch $smoke 'COMPLETION_BASE_GAPS[\s\S]{0,4000}COMPLETION_DEFERRED_ACTIVE_RECOVERY_BASE_GAPS[\s\S]{0,4000}COMPLETION_DONE_TASK_GAPS[\s\S]{0,4000}COMPLETION_CANCEL_TASK_GAPS' 'Prod-like smoke must measure actionable and recovery-deferred completion backlogs.'
Assert-TextMatch $smoke 'AdditionalRoles\s+@\("OWNER"\)' 'Workload smoke must use the allowlisted ADMIN client with temporary OWNER authority for owner-only settings.'
Assert-TextMatch $smoke '\$adminOnlyHeaders[\s\S]{0,500}StatusCode\s+-ne\s+403' 'Workload smoke must recreate a clean ADMIN token for the negative live-activation check.'
...
Assert-TextMatch $restore 'Import-LocalMySqlGzipStream -ComposeArguments \$composeArgs -Path \$resolvedDumpPath -ExpectedSha256 \$verifiedDumpSha256' 'MySQL import must use the verified host gzip stream.'
Assert-TextMatch $restore '\$verification\.Read\([\s\S]{0,2500}\$process\.Start\(\)' 'The complete gzip stream must be validated before starting the MySQL import.'
Assert-TextMatch $restore '\$gzip\.CopyToAsync\(\$process\.StandardInput\.BaseStream\)' 'MySQL import must preserve binary bytes without shell text conversion.'
Assert-TextMatch $restore '\$copy\.GetAwaiter\(\)\.GetResult\(\)' 'MySQL import must propagate decompression and stream write failures.'
Assert-TextMatch $restore '\$process\.ExitCode -ne 0' 'MySQL import must reject a failed database client.'
...
Assert-TextMatch $smoke "'workload[.]live[.]mode', 'SHADOW'[\s\S]{0,300}'workload[.]live[.]apply-enabled', 'false'" 'Prod-like smoke must force restored workload live mode off before running local safety checks.'
Assert-TextMatch $restore "'workload[.]live[.]mode', 'SHADOW'[\s\S]{0,300}'workload[.]live[.]apply-enabled', 'false'" 'Prod-like restore must force restored workload live mode off for local developer stacks.'
...
Assert-TextMatch $smoke 'MIGRATIONS=16[\s\S]{0,1000}HARDENING_MIGRATIONS=5[\s\S]{0,1000}FUTURE_EXPIRY_EVENTS=0' 'Contractor payment smoke must prove V217-V232 and all financial hardening migrations through V249 plus expiry-event timeline integrity.'
Assert-TextMatch $smoke "ACCOUNTING_SHADOW=[\s\S]{0,500}ACCOUNTING_LIVE=[\s\S]{0,500}ROLLOUT_LEGACY=[\s\S]{0,1000}ROLLOUT_COMPLETION=[\s\S]{0,1000}ROLLOUT_PAYMENT=[\s\S]{0,8000}LIVE_ALLOCATIONS=[\s\S]{0,500}CUTOVER_ROWS=" 'Contractor payment smoke must collect enough facts to distinguish pre-cutover and post-cutover rollout states, including financial integrity checks between rollout and allocation facts.'
Assert-TextMatch $smoke '\$legacyShadowState\s*=\s*\([\s\S]{0,1000}\$factValues\["ACCOUNTING_SHADOW"\]\s+-eq\s+1[\s\S]{0,1000}\$factValues\["LIVE_ALLOCATIONS"\]\s+-eq\s+0[\s\S]{0,1000}\$factValues\["CUTOVER_ROWS"\]\s+-eq\s+0' 'Contractor payment smoke must still accept the pre-cutover LEGACY/SHADOW state.'
Assert-TextMatch $smoke '\$paymentLiveState\s*=\s*\([\s\S]{0,1000}\$factValues\["ACCOUNTING_LIVE"\]\s+-eq\s+1[\s\S]{0,1000}\$factValues\["ROLLOUT_COMPLETION"\]\s+-eq\s+0[\s\S]{0,1000}\$factValues\["ROLLOUT_PAYMENT"\]\s+-eq\s+1[\s\S]{0,1000}\$factValues\["CUTOVER_ROWS"\]\s+-eq\s+1' 'Contractor payment smoke must accept only the immutable post-cutover PAYMENT/LIVE accounting authority.'
Assert-TextMatch $smoke 'GENERATION_COLLATIONS=5' 'Contractor payment smoke must keep collation-safe generation joins.'
Assert-TextMatch $smoke 'live_master=false[\s\S]{0,500}reward_master=false' 'Contractor payment smoke must keep deployment LIVE masters closed in prod-like local mode.'
Assert-TextMatch $smoke "'contractor_legacy_reward_reconciliation_runs'[\s\S]{0,300}'contractor_legacy_reward_reconciliation_items'" 'Prod-like smoke must require both immutable legacy-reconciliation audit tables.'
Assert-TextMatch $smoke '"REQUIRED_TABLES=8"' 'Prod-like smoke must require the complete contractor accounting schema including both reconciliation tables.'
Assert-TextMatch $smoke '\$completionGapFacts\s*=\s*@\([\s\S]{0,500}\^COMPLETION_\(BASE\|DEFERRED_ACTIVE_RECOVERY_BASE\|DONE_TASK\|CANCEL_TASK\)_GAPS=\[0-9\]\+\$[\s\S]{0,500}\$completionGapFacts\.Count\s+-ne\s+4' 'Contractor payment smoke must parse actionable, recovery-deferred, DONE-task, and cancellation backlog counts.'
...
Assert-TextMatch $restore 'contractor_actual_payment_attributions SET original_recipient_name_snapshot = NULL, actual_recipient_name_snapshot = NULL, receipt_url = NULL' 'Prod-like restore must scrub immutable actual-payment recipient and receipt envelopes before using a local-only encryption key.'
Assert-TextMatch $restore 'payment_links SET manual_actual_original_recipient_name_snapshot = NULL, manual_actual_recipient_name_snapshot = NULL, manual_actual_receipt_url = NULL' 'Prod-like restore must scrub immutable payment-link attribution envelopes before using a local-only encryption key.'
Assert-TextMatch $restore 'manual_payment_task_ledger_entries SET accounting_target_label_snapshot = NULL, manual_phone_snapshot = NULL, bank_recipient_name_snapshot = NULL, manual_bank_name_snapshot = NULL, manual_payment_url_snapshot = NULL' 'Prod-like restore must scrub immutable manual-payment ledger envelopes before using a local-only encryption key.'
Assert-TextMatch $smoke 'Contractor allocation quarantined for retry:[\s\S]{0,120}code=JpaSystemException[\s\S]{0,300}Credential encryption key is unavailable' 'Prod-like log checks must fail on unreadable contractor attribution envelopes.'
...
Assert-TextMatch $nginx '(?ms)location /internal/outreach/v1/ \{.*?access_log off;.*?proxy_pass http://otziv_app;' 'Production Nginx must proxy the token-protected outreach bridge without logging its requests.'
Assert-TextMatch $localNginx '(?ms)location /internal/outreach/v1/ \{.*?access_log off;.*?proxy_pass http://otziv_app;' 'Prod-local Nginx must exercise the outreach bridge route.'
...
Assert-TextMatch $productionProperties 'multibrowser\.enabled=\$\{MULTIBROWSER_ENABLED:false\}' 'The external MultiBrowser integration must remain disabled by default.'
Assert-TextMatch $productionCompose 'MULTIBROWSER_ENABLED:\s*\$\{MULTIBROWSER_ENABLED:-false\}' 'Production Compose must keep MultiBrowser opt-in.'
Assert-TextMatch $productionCompose 'OTZIV_MULTIBROWSER_PUBLIC_UPSTREAM:\s*\$\{MULTIBROWSER_PUBLIC_UPSTREAM:-https://127\.0\.0\.1:9\}' 'Disabled MultiBrowser VNC routing must fail locally without external egress.'
Assert-TextMatch $productionCompose 'OTZIV_MULTIBROWSER_PUBLIC_HOST:\s*\$\{MULTIBROWSER_PUBLIC_HOST:-disabled\.invalid\}' 'Disabled MultiBrowser VNC routing must use a non-routable TLS name.'
...
Assert-TextNotMatch $backendDockerfile 'outreach-module|-pl backend|-am clean package' 'The backend image must remain independent from the extracted outreach reactor.'
