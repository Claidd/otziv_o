...
Assert-TextMatch $deploy '(?m)^compose run --rm --no-deps --interactive=false -T --cap-add CHOWN --user 0 --entrypoint sh whatsapp_lika ' 'The one-shot WhatsApp ownership migration must regain CAP_CHOWN after the service drops all capabilities.'
