...
try {
    ...
    foreach ($name in @('allowList', 'reviewedSql004', 'reviewedArchiveOrder')) {
    ...
    foreach ($rule in @('SQL001', 'SQL002', 'SQL003')) {
        if (Test-AllowListed -RelativePath $reviewedArchiveOrder.Path -Line $reviewedArchiveOrder.Lines[0] -SourceSha256 $reviewedArchiveOrder.SourceSha256 -RuleId $rule) {
            throw "Archive SQL004 review must not exempt $rule"
        }
        $checks++
    }

    # A separate fixture proves that changing the enum provenance invalidates
    # the review even when the ORDER BY expression itself is unchanged.
    [IO.File]::WriteAllText($foreign, 'class ForeignMaintenance {}', $utf8)
    [IO.File]::WriteAllText($fixtureSource, $source, $utf8)
    $archiveSource = [IO.File]::ReadAllText((Join-Path $repoRoot $reviewedArchiveOrder.Path))
    $fixtureArchive = Join-Path $fixtureRoot $reviewedArchiveOrder.Path
    $null = New-Item -ItemType Directory -Force -Path (Split-Path $fixtureArchive)
    [IO.File]::WriteAllText($fixtureArchive, $archiveSource, $utf8)
    Invoke-FixtureGit -Arguments @('add', '--', 'backend/src/main/java')
    Assert-Guard -Mode dir -ShouldPass $true -Scenario 'Reviewed two-value archive sort is accepted'
    Assert-Guard -Mode staged -ShouldPass $true -Scenario 'Reviewed archive source in index is accepted'
    $unsafeArchive = $archiveSource.Replace(
        'direction == org.springframework.data.domain.Sort.Direction.ASC ? "ASC" : "DESC"', 'search')
    if ($unsafeArchive -ceq $archiveSource) { throw 'Counterfactual must replace the enum whitelist with request text.' }
    [IO.File]::WriteAllText($fixtureArchive, $unsafeArchive, $utf8)
    Assert-Guard -Mode dir -ShouldPass $false -Scenario 'Raw request text replacing archive whitelist is rejected'
    Assert-Guard -Mode staged -ShouldPass $true -Scenario 'Archive index is independent of unsafe working-tree provenance'
    Invoke-FixtureGit -Arguments @('add', '--', $reviewedArchiveOrder.Path)
    [IO.File]::WriteAllText($fixtureArchive, $archiveSource, $utf8)
    Assert-Guard -Mode staged -ShouldPass $false -Scenario 'Unsafe archive index cannot use the safe working-tree digest'
    $archiveLf = $archiveSource.Replace("`r`n", "`n")
    foreach ($text in @($archiveLf, $archiveLf.Replace("`n", "`r`n"))) {
        [IO.File]::WriteAllText($fixtureArchive, $text, $utf8)
        Invoke-FixtureGit -Arguments @('add', '--', $reviewedArchiveOrder.Path)
        Assert-Guard -Mode staged -ShouldPass $true -Scenario 'Archive review is invariant to LF/CRLF'
    }
    [IO.File]::WriteAllText($fixtureArchive, $archiveSource + "`n" + 'var other = "SELECT id FROM anything ORDER BY " + search;', $utf8)
    Assert-Guard -Mode dir -ShouldPass $false -Scenario 'Another query in archive repository is not exempt'
