...
summary = {'schema':'otziv-c15-alloy-closure-analysis-v1','createdAt':datetime.datetime.now(datetime.UTC).isoformat(),'publishedBinarySha256':published['binarySha256'],'reproducedBinaryByteIdentical':True,'moduleProvenanceByteIdentical':True,'buildInfoByteIdenticalIgnoringFilename':True,'packageCount':len(packages),'importEdges':len(edges),'daemonPackages':0,'linkedFunctions':linked['functionCount'],'daemonFunctions':len(daemon_functions),'sourceArchiveSha256':'69efdb87a91bb538323f5ccb6bcd86240cee3a78ee97632bd1005c434344c7f1','originalDockerfileSha256':'240aa5e4206a5a6076d9de6cb522e24672fa9b3bc98ab619cad2484286ae546b','rawGoListSha256':sha(raw),'importGraphSha256':sha(graph_bytes),'decision':'EXACT_PUBLISHED_ARTIFACT_HAS_NO_DAEMON_PACKAGE; SECOND_PARSER_VERIFICATION_REQUIRED_BEFORE_GATE_INTEGRATION','files':{p.name:{'bytes':p.stat().st_size,'sha256':sha(p.read_bytes())} for p in [BASE/'import-closure.json.gz',BASE/'packages.raw.jsons.gz',BASE/'linked-functions.raw.json.gz']}}
