...
for(const [name,path,mutate]of [
  ...
  ['unpatched library','rehearsal.json',v=>v.candidateLibevent='libevent-2.1.12-8.el9_4.x86_64'],
  ...
  const p='infrastructure/runtime-security/proofs/c18-mysql/'+path,value=JSON.parse(await read(p));mutate(value);
