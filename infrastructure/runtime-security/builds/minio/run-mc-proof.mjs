...
const prefix = `otziv-c14-mc-${owner.slice(0, 8)}`;
...
async function cli(script, extraEnv = {}) {
  ...
  await docker(['create', '--name', name, '--label', `${label}=${owner}`, '--network', network, '--read-only', '--cap-drop=ALL', '--security-opt=no-new-privileges:true', '--tmpfs', '/tmp:rw,nosuid,size=32m', '--memory', '192m', '--cpus', '1', '--pids-limit', '96', '-e', 'MINIO_ROOT_USER', '-e', 'MINIO_ROOT_PASSWORD', ...Object.keys(extraEnv).flatMap(k => ['-e', k]), '--entrypoint', '/bin/sh', image, '-ec', 'mc alias set fixture http://minio-proof:9000 "$MINIO_ROOT_USER" "$MINIO_ROOT_PASSWORD" >/dev/null\n' + script]);
  resources.push({ kind: 'container', name });
  const result = await docker(['start', '-a', name]);
...
const result = { schema: 'otziv-c14-mc-compatibility-v1', result: failure ? 'FAIL' : 'PASS', timestamp: new Date().toISOString(), image, baselineServer: baseline, syntheticOnly: true, checks, cleanup, failure, limits: ['CLI compatibility with the pinned current standalone server; full server upgrade and physical backup restoration are a separate proof.'] };
