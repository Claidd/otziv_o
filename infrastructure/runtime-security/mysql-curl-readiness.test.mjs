...
import {validateDatabaseTransitionReadiness,MYSQL_REFRESH_REFERENCE} from './mysql-curl-readiness.mjs';
...
for(const [name,path,mutate]of [
  ...
  ['unpatched library','rehearsal.json',v=>v.candidateCurl=['curl-7.76.1-40.el9_8.5.x86_64']],
  ['unrelated package change','rehearsal.json',v=>v.candidateOtherPackagesSha256='0'.repeat(64)],
  ...
  const p='infrastructure/runtime-security/proofs/c21-mysql/'+path,value=JSON.parse(await read(p));mutate(value);
