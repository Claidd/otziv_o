"""Read-only VPS capture and isolated Keycloak cryptographic-library patch rehearsal.
...
OWNER = 'otziv-kc-c19-' + uuid.uuid4().hex[:12]
LABEL = 'com.otziv.kc-c19-rehearsal.owner'
OLD_PG = 'ghcr.io/claidd/otziv-security@sha256:a30a580feb45fce65b87d21a104d407d3d1c5b255ae0e9a05ce9903e10adb270'
PG_CONFIG = 'sha256:9aff3268aa60897853557e45444da2f6c1d45e311eb543e9eb8e0f49339c5f6e'
SOURCE_KC = 'ghcr.io/claidd/otziv-security@sha256:bd5687843becb0cdb232c2b864cc3786fafb8fc91f30f531fa2db7fc4d1fc59d'
KC_CONFIG = 'sha256:34e7587586d4d0a8d8330845f43cc58db9196d4dc6c4f120ac70519541453e5e'
NEW_PG = OLD_PG
NEW_KC = args.candidate
containers, volumes, network, stage = [], [], None, 'setup'
REPORT = {'schema': 'otziv-keycloak-c19-rehearsal-v1', 'result': 'FAIL', 'checks': [],
          ...
          'keycloak': SOURCE_KC, 'keycloakConfig': KC_CONFIG,
...
'''
        ...
        import re
        if not re.fullmatch(r'ghcr\.io/claidd/otziv-security@sha256:[a-f0-9]{64}', NEW_KC or ''):
        ...
        REPORT.update(candidate=NEW_KC, candidateLocalId=inspect(NEW_KC, 'image')['Id'],
        ...
        docker(['run', '-d', '--name', runner, '--label', LABEL + '=' + OWNER, '--network', network,
                ...
                '--cpus', '.25', '--pids-limit', '64', '--entrypoint', 'node', 'node:22-bookworm-slim@sha256:83f487e0a63425e5b4d146fb5e5be574bcbe1b7b843d3ebafdd95eaf7767a7e5',
        ...
        source_issuer = start_kc('source-kc', SOURCE_KC, kcenv, runner, before['admin'])
        check('source_existing_password_login', REPORT['keycloakChecks']['source-kc']['adminRead'])
        check('source_roles_and_credentials_preserved', local_critical(old) == before['critical'])
        stop(source_issuer)
        original = local_tables(old)
        schema_sql = "SELECT coalesce(jsonb_agg(to_jsonb(t) ORDER BY table_name,ordinal_position),'[]'::jsonb) FROM information_schema.columns t WHERE table_schema='public'"
        ...
        local_sql(current, "CREATE TABLE otziv_c19_fixture(id bigint PRIMARY KEY,value text); INSERT INTO otziv_c19_fixture VALUES(1,'Иркутск—Unicode—ß—東京');")
        schema_before = local_sql(current, schema_sql)
        ...
        stop(issuer)
        check('crypto_patch_no_schema_migration', local_sql(current, schema_sql) == schema_before)
        ...
        old_issuer = start_kc('same-volume-old-kc', SOURCE_KC, kcenv, runner, before['admin'])
        check('same_volume_old_issuer_login', REPORT['keycloakChecks']['same-volume-old-kc']['adminRead'])
        check('same_volume_old_issuer_credentials_preserved', local_critical(rolled_back) == before['critical'])
        stop(old_issuer)
        stop(rolled_back)
        ...
        restored_issuer = start_kc('restored-kc', SOURCE_KC, kcenv, runner, before['admin'])
