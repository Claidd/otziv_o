...
export const test = base.extend({
  fixture: async ({ context, baseURL, browser }, use, info) => {
    ...
    const state = {
      ...
      api: new Map(), tokenGrants: [], respondToToken: null, completeMobileLogin: false,
    ...
    await context.route('**/*', async (route) => {
      ...
      if (url.pathname.endsWith('/protocol/openid-connect/auth')) {
        if (state.completeMobileLogin && state.roles && url.searchParams.get('client_id') === 'otziv-mobile') {
          const target = new URL(url.searchParams.get('redirect_uri'));
          if (target.origin !== origin || target.pathname !== '/auth/callback') throw new Error('Mobile fixture callback must stay on loopback');
          expect(url.searchParams.get('code_challenge_method')).toBe('S256');
          expect(url.searchParams.get('code_challenge')).toMatch(/^[A-Za-z0-9_-]{43}$/);
          target.search = new URLSearchParams({ code: 'fixture-code', state: url.searchParams.get('state') }).toString();
      ...
      if (url.pathname.endsWith('/protocol/openid-connect/token') && state.roles) {
        const tokenForm = new URLSearchParams(request.postData());
        const grant = tokenForm.get('grant_type');
        state.tokenGrants.push(grant);
        if (state.respondToToken) {
          const handled = await state.respondToToken(route, grant);
          if (handled) return;
        }
        const now = Math.floor(Date.now() / 1000);
        const claims = { sub: 'browser-fixture-user', sid: 'fixture-session', session_state: 'fixture-session',
          iss: `${origin}/keycloak/realms/otziv`, aud: tokenForm.get('client_id') || 'otziv-frontend', azp: tokenForm.get('client_id') || 'otziv-frontend',
      ...
      if (url.pathname.endsWith('/protocol/openid-connect/logout')) {
        const target = new URL(url.searchParams.get('post_logout_redirect_uri') || '/auth/restart', origin);
        if (target.origin !== origin) throw new Error('OIDC fixture logout must stay on loopback');
        return route.fulfill({ status: 302, headers: { location: target.href } });
      }
